...
carMarket.getAgencyIdByName = function (name) {
  // this.sellers = []
  // agency = object of agency or undefined
  const agency = this.sellers.find((agencyObject) => {
    return name === agencyObject.agencyName;
  });
...
//console.log(carMarket.getAgencyIdByName("The Auto World"));

//* getAllAgenciesName
//? @param {}
//? @return {string[]} - agenciesNameArr - Array of all agencies name

carMarket.getAllAgenciesName = function (object) {
  let agenciesNameArr = [];
  this.sellers.map((agency) => {
...
carMarket.getAllCarToBuy = function () {
  let allCarsToBuy = [];
  this.sellers.map((carObject) => {
    allCarsToBuy.push(carObject.cars);
  });
...
carMarket.getAllCarToBuyByAgencyId = function (agency_id) {
  let carsArray = [];

  const agency = this.sellers.find((agencyObject) => {
    return agency_id === agencyObject.agencyId;
  });

  if (agency === undefined) {
    return agency;
  }
  agency.cars.map((carObject) => carsArray.push(carObject.models));
...
carMarket.getAllBrandsToBuyAgencyId = function (agency_id) {
  let arrOfBrands = [];
  ...
  agency.cars.map((carObject) => arrOfBrands.push(carObject.brand));
...
carMarket.getCustomerByName = function (name) {
  const customer_agency = this.customers.find((customerObject) => {
    return name === customerObject.name;
  });
  if (customer_agency === undefined) {
    return undefined;
  }
...
//console.log(carMarket.getCustomerByName("Will Reyes"));
...
carMarket.getCustomerIdByName = function (name) {
  ...
  return customer_agency.id;
};

//console.log(carMarket.getCustomerIdByName("Bob Steel"));
...
carMarket.getAllCustomersNames = function () {
  ...
  this.customers.map((customerObject) => {
    customersNameArr.push(customerObject.name);
  });
...
carMarket.getAllCustomerCars = function (customer_id) {
  // let customerCarsArr = [];

  const customer = this.customers.find((customerObject) => {
    return customer_id === customerObject.id;
  });
...
//console.log(carMarket.getAllCustomerCars("pAuFtn_WA"));

//* getCustomerCash
//? @param {id} - costumerId - costumer id
//? @return {number} - CustomerCash

carMarket.getCustomerCash = function (customer_id) {
  const customer = this.customers.find((customerObject) => {
    return customer_id === customerObject.id;
  });
...
carMarket.setPropertyBrandToAllCars = function () {
  this.sellers.map((agencyObject) => {
    agencyObject.cars.map((carObject) => {
      carObject.models.map((model) => {
        model.brand = carObject.brand;
      });
    });
  });
};
...
carMarket.setNewCarToAgency = function (agency_id, carObject) {
  let agency = this.sellers.find((agencyObject) => {
    return agencyObject.agencyId === agency_id;
  });
  agency.cars.push(carObject);
};

//carMarket.setNewCarToAgency("Plyq5M5AZ", { brand: "toyotta" });
...
carMarket.deleteCarFromAgency = function (agency_id, car_id) {
  const agency = this.sellers.find((agenceObject) => {
    return agency_id === agenceObject.agencyId;
  });
  //console.log(agency);
  for (let carObject of agency.cars) {
    for (let i = 0; i < carObject.models.length; i++) {
      // const car_model = carObject.models.find((modelObject) => {
      if (car_id === carObject.models[i].carNumber) {
        carObject.models.splice(i, 1);
        return;
      }
    }
  }
...
//console.log(carMarket.sellers[0].cars[0].models[0]);
...
carMarket.decrementOrIncrementCashOfAgency = function (agency_id, amount) {
  const agency = this.sellers.find((agencyObject) => {
    return agency_id === agencyObject.agencyId;
  });
  if (agency === undefined) return undefined;

  return agency.cash + amount;
};

//console.log(carMarket.decrementOrIncrementCashOfAgency("Plyq5M5AZ", -4000));
...
carMarket.decrementOrIncrementCreditOfAgency = function (agency_id, amount) {
  const agency = this.sellers.find((agencyObject) => {
    return agency_id === agencyObject.agencyId;
  });
  if (agency === undefined) return undefined;
...
carMarket.setAmountOfCarsToBuyToAllAgency = function () {
  for (let agency of this.sellers) {
    let countCarsPerAgency = 0;
    for (let car of agency.cars) {
      for (models of car.models) {
        countCarsPerAgency++;
      }
    }
    agency.amounOfCars = countCarsPerAgency;
  }
...
carMarket.setCarToCostumer = function (customer_id, car_Object) {
  customer = this.customers.find((customerObject) => {
    return customer_id === customerObject.id;
  });

  if (customer === undefined) return undefined;

  customer.cars.push(car_Object);
  return customer.cars;
};

// console.log(
//   carMarket.setCarToCostumer("2RprZ1dL", {
//     name: "M4",
//     year: 2018,
//     price: 60000,
//     carNumber: "gt6g",
...
carMarket.deleteCarOfCostumer = function (customer_id, car_id) {
  const customer = this.customers.find((customerObject) => {
    return customer_id === customerObject.id;
  });
  if (customer === undefined) return undefined;
  ...
  for (let i = 0; i < customer.cars.length; i++) {
    if (customer.cars[i].carNumber === car_id) {
      customer.cars.splice(i, 1);
      break;
    }
  }
  return customer.cars;
...
//console.log(carMarket.deleteCarOfCostumer("FQvNsEwLZ", "vaJvd"));
...
carMarket.decrementOrIncrementCashOfCostumer = function (customer_id, amount) {
  const customer = this.customers.find((customerObject) => {
    return customerObject.id === customer_id;
  });
